...
const [filter, setFilter] = React.useState(() => charFiltering.Chars);
  ...
  React.useEffect(() => {
    ...
    if (attributes.length === 0) {
      setFilter(charFiltering.setFilterByAttribute(["Fire", "Water", "Wind", "Thunder", "Dark", "Light"]))
    }
  ...
  React.useEffect(() => {
    ...
    if (rarity.length === 0) {
      setFilter(charFiltering.setFilterByRarity(["Five", "Four", "Three", "Two", "One"]))
    }
  ...
  React.useEffect(() => {
    ...
    console.log(term)
    // console.log("Filter Check", charFiltering.setFilterByRarity(rarity, filter));
    setFilter(charFiltering.Chars.filter((char) => char.ENName.toLowerCase().includes(term.toLowerCase())))
    if (term.length === 0) {
  ...
  return (
    ...
    <Stack
      ...
      {...(isDesktop
        ...
        : { direction: 'column' })}
    >
            <TextField id="outlined-basic" label="Search" variant="outlined" onChange={(e)=>{setTerm(e.target.value)}}/>
      ...
      >
        ...
        >
          <ToggleButton value="Five">
            <Image src={Five} alt="Five" width={rarityWidth} height="auto"/>
          </ToggleButton>
          <ToggleButton value="Four">
            <Image src={Four} alt="Four" width={rarityWidth} height="auto"/>
          </ToggleButton>
          <ToggleButton value="Three">
            <Image src={Three} alt="Three" width={rarityWidth} height="auto"/>
          </ToggleButton>
          <ToggleButton value="Two">
            <Image src={Two} alt="Two" width={rarityWidth} height="auto"/>
          </ToggleButton>
          <ToggleButton value="One">
            <Image src={One} alt="One" width={rarityWidth} height="auto"/>
          </ToggleButton>
        </StyledToggleButtonGroup>
      </Paper>
      ...
      }} fullWidth={isDesktop ? false : true} variant="outlined" style={{marginTop: "15px"}}>Reset Filter</Button>
      ...
      <div>{
        ...
        <div>
            <Box sx={{ flexGrow: 1, height: { xs: '400px', md: 'auto' }, overflowY: 'auto' }}
            md={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-around', alignItems: 'center' }}
            >
          <Grid container 
            ...
            {filter.map((char) => {
                return (
                  <ToolTip title={char.ENName}>
                        ...
                        >
                          <img src={charFiltering.getCharIcon(char.DevNicknames)} alt={char.DevNicknames} width={50} height={50}/>
                        </Item>
                  </ToolTip>
                );
            })}
